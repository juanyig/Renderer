...
// log ray miss
            ...
            if(envLight != NULL) return envLight->sample_dir(r);
...
// log ray hit
        ...
        if (!isect.bsdf->is_delta()) {
            ...
            for (SceneLight* light : scene->lights) {
                ...
                for (int i = 0; i < num_light_samples; i++) {
                    ...
                    Ray shadow = Ray(hit_p + EPS_D * dir_to_light, dir_to_light, dist_to_light);
        ...
        Vector3D wi_o,wi;
        float pdf = 0.0;
        Spectrum f = isect.bsdf->sample_f(w_out, &wi_o, &pdf);
        wi = (o2w * wi_o).unit();
        float terminateProbablity = 1.0f - f.illum();
        terminateProbablity = clamp(terminateProbablity, 0.0, 1.0);

        //clamp direct lighting L_out
        double r_tmp = L_out.r;
        double g = L_out.g;
        double b = L_out.b;
        r_tmp = clamp(r_tmp, 0.0, 1.0);
        g = clamp(g, 0.0, 1.0);
        b = clamp(b, 0.0, 1.0);

        // (2) potentially terminate path (using Russian roulette)
        float rand = (float)std::rand()/RAND_MAX;
        if (rand < terminateProbablity)
        {
            return Spectrum(r_tmp, g, b);
        }

        // (3) evaluate weighted reflectance contribution due
        // to light from this direction
        Ray recur = Ray(hit_p + EPS_D * wi, wi, int(r.depth+1));
        double tmp = 1.0f / (pdf * (1.0f - terminateProbablity));
        double dot = wi[0] * hit_n[0] + wi[1] * hit_n[1] + wi[2] * hit_n[2];

        return Spectrum(r_tmp, g, b) + f * trace_ray(recur) * abs(dot) * tmp;
        //clamp?
//  double r_tmp = (L_out + f * trace_ray(recur) * abs(dot) * tmp).r;
//  double g = (L_out + f * trace_ray(recur) * abs(dot) * tmp).g;
//  double b = (L_out + f * trace_ray(recur) * abs(dot) * tmp).b;
//  r_tmp= clamp(r_tmp, 0.0, 1.0);
//  g = clamp(g, 0.0, 1.0);
//  b = clamp(b, 0.0, 1.0);
//  return Spectrum(r_tmp, g, b);
    ...
    Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
        ...
        for (int i = 0; i < num_samples; i++)
        {
            ...
            if (i == 0) offset = Vector2D(0.5, 0.5);

            Vector2D p = Vector2D((x + offset.x)/w, (y + offset.y)/h);
            Spectrum sample = trace_ray(camera->generate_ray(p.x - 0.5, p.y - 0.5));
            color += sample;
        }
        color *= 1.0f/num_samples;
        return color;
    }
