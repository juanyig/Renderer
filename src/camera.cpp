...
Ray Camera::generate_ray(double x, double y) const {
  ...
  Vector3D camera_ori (0, 0, 0);
  Vector3D world_ori = c2w * (camera_ori) + pos;

  double sensor_h = 2.0 * tan(radians(vFov / 2.0));
  double ratio = 1.0 * screenW / screenH;
  double sensor_w = sensor_h * ratio;

  Vector3D camera_dir (x * sensor_w, y * sensor_h, -1);
  Vector3D world_dir = c2w * (camera_dir);

  return Ray(world_ori, world_dir.unit());
}
